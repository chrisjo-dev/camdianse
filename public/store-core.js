...
export function buildSeed(todayKey) {
  const ingredients = [
    {
  ...
  const pharmacyProductStatuses = [
    { pharmacyId: "pharm-riverside", productId: "prd-panadol-500", status: "GREEN", updatedAt: timestamp("06:30") },
    { pharmacyId: "pharm-riverside", productId: "prd-tylenol-500", status: "YELLOW", updatedAt: timestamp("06:35") },
    { pharmacyId: "pharm-riverside", productId: "prd-ibuprofen-200", status: "RED", updatedAt: timestamp("06:40") },
    { pharmacyId: "pharm-bkk1", productId: "prd-panadol-500", status: "GREEN", updatedAt: timestamp("07:10") },
    { pharmacyId: "pharm-bkk1", productId: "prd-tylenol-500", status: "RED", updatedAt: timestamp("07:12") },
    { pharmacyId: "pharm-bkk1", productId: "prd-imodium", status: "YELLOW", updatedAt: timestamp("07:14") },
    { pharmacyId: "pharm-aeon", productId: "prd-panadol-500", status: "GREEN", updatedAt: timestamp("08:10") },
    { pharmacyId: "pharm-aeon", productId: "prd-tylenol-500", status: "GREEN", updatedAt: timestamp("08:15") },
    { pharmacyId: "pharm-aeon", productId: "prd-ibuprofen-200", status: "YELLOW", updatedAt: timestamp("08:20") },
  ];
...
export function createStore({ now = new Date(), uuid = () => crypto.randomUUID(), initialState } = {}) {
  const state = structuredClone(initialState ?? buildSeed(getDateKey(now)));
  ...
  return {
    ...
    searchCatalog(query) {
      ...
      return { query, results: [...ingredientMatches, ...directProductMatches] };
    },
    ...
    getNearbyPharmacies({ productId, lat, lng, radiusMeters = 1000 }) {
      ...
      const pharmacies = state.pharmacyProductStatuses
        ...
        .map((entry) => {
          ...
          return {
            ...
            lat: pharmacy.lat,
            lng: pharmacy.lng,
            distanceMeters: haversineDistanceMeters(lat, lng, pharmacy.lat, pharmacy.lng),
      ...
      return { product, radiusMeters, pharmacies };
    },
    ...
    getPharmacyCatalog(pharmacyId) {
      ...
      return {
        pharmacy,
        catalog: state.pharmacyProductStatuses
    ...
    updatePharmacyProductStatus({ pharmacyId, productId, status, updatedAt = new Date().toISOString() }) {
      ...
      return { pharmacyId, productId, status, updatedAt };
    },
    ...
    getInquiry(inquiryId) {
      ...
      return {
        ...inquiry,
        pharmacy: pharmacyById.get(inquiry.pharmacyId),
        product: getProductSummary(inquiry.productId),
      };
    },
